...
import {Applytoken, ApplyToken} from "./ApplyToken";
...
export default class InsightFacade implements IInsightFacade {
    ...
    public addDataset(id: string, content: string, kind: InsightDatasetKind): Promise<string[]> {
        let ids = this.ids;
        let localClass = this;
        let dataset: IDataSet = new IDataSet();
        ...
        return new Promise<string[]>(function (resolve, reject) {
            ...
            for (let id1 of ids) {
                if (id === id1) {
                    return reject( new InsightError("Duplicate datasets"));
                }
            }
            if (kind === InsightDatasetKind.Courses) {
                zip.loadAsync(content, {base64: true}).catch(function () {
                    return reject(new InsightError("Not a valid zip file1"));
                }).then(function (result1: JSZip) {
                    // try {
                    result1.folder("courses").forEach(function (path, file) {
                        promises.push(file.async("text"));
                    });
                    /*} catch (e) {
                        let er = new InsightError("Not a valid zip file3");
                        return reject(er);
                    }*/
                    Promise.all(promises).then(function (result) {
                        ...
                        for (const res of result) {
                            ...
                            try {
                                ...
                                for (const section of course) {
                                    let isection: ISection = new ISection();
                        ...
                        if (courses.length < 1) {
                            return reject(new InsightError("no valid course section"));
                        }
                        ids.push(id);
    ...
    private htmlRecursion(json: any, htmlArray: string[]) {
    ...
    private parseBuilding(html: any, buildingData: Array<{ [id: string]: string }>) {
    ...
    private parseRoom(html: any, rooms: IRoom[]) {
        if (html.hasOwnProperty("tagName") && html.tagName === "tr" && html.hasOwnProperty("childNodes") &&
            ...
            newRoom.iseats = Number(html.childNodes[3].childNodes[0].value.substring(13,
                                                                html.childNodes[3].childNodes[0].value.length - 10));
            newRoom.ifurniture = html.childNodes[5].childNodes[0].value.substring(13,
                                                                html.childNodes[5].childNodes[0].value.length - 10);
            newRoom.itype = html.childNodes[7].childNodes[0].value.substring(13,
                                                                html.childNodes[7].childNodes[0].value.length - 10);
    ...
    private handleGeo(url: string): Promise<any> {
        let lat = 0;
        let lon = 0;
    ...
    public removeDataset(id: string): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            if (id === "") {
                return reject(new NotFoundError("Empty id to delete"));
            }
            if (id === undefined || id === null) {
                return reject(new InsightError("Empty id to delete"));
            }
            let index = 0;
            ...
            if (index === (this.insightDatasets.length)) {
                return reject(new NotFoundError("No such dataset"));
            }
    ...
    public performQuery(query: any): Promise <any[]> {  // Problem of *, Problem of assign to new Dataset
        let local = this;
        return new Promise<any[]>((resolve, reject) => {
            let result: any[];
            ...
            if ((!query.OPTIONS.hasOwnProperty("COLUMNS")) || query.OPTIONS.COLUMNS == null)  {
                return reject(new InsightError("Incorrect file"));
            }
            ...
            try {
                ...
                if (id === null) {
                    return reject(new InsightError("No such dataset"));
                }
                for (let dataset of this.datasets) {
                    if (dataset.iid === id) {
                        kind = dataset.ikind; // get the kind of the current dataset
                    }
                }
                if (Object.keys(where).length > 1) {
                return reject (new InsightError("Invalid where"));
                }
                // result = this.numberTodataset(this.parsewhere(where, id), id).ielements;
                if (Object.keys(where).length === 0) {
                    for (let dataset of this.datasets) {
                        if (dataset.iid === id) {
                            if (dataset.ikind === InsightDatasetKind.Courses) {
                    ...
                    // throw new InsightError("No datasets");
                } else {
                    if (kind === "courses") {
                        result = this.numberTodataset(this.parsewhere(where, id), id).ielements;
                    }
                    if (kind === "rooms") {
                        let str: string[] = this.parsewhere(where, id);
                        // global.console.log(str);
                       // result = this.stringTodataset(this.parsewhere(where, id), id).ielements;
                ...
                if (options.hasOwnProperty("ORDER") && options.hasOwnProperty("COLUMNS")) {
                    if (Object.keys(options.ORDER).length < 2 && ! options.COLUMNS.includes(options.ORDER)) {
                        return reject(new InsightError("Order is not in Column"));
                    }
                    if (Object.keys(options).length > 2) {
                        return reject(new InsightError("Invalid Options"));
                    } else {
                    ...
                    return reject(new InsightError("Invalid Option field"));
                }
                if (result.length > 5000) {
                    return reject(new InsightError("Too much"));
                }
            } catch (e) {
    ...
    private parseTransformations(trans: any, res: any[], id: string): any[] { // res is the result from parseWhere
        let result = null;
    ...
    private parseGroup(group: any, res: any[], id: string): any[] {
        if (group.length < 1) {
            return res;
        }
        ...
        for (let rGroup of res) {
            ...
            for (let element of rGroup) {
                if (!resultMap.has(element.get(groupKey))) {       // if the map doesn't have this value,add it
    ...
    private parseApply(group: any, res: any[], apply: any, id: string): any[] {
        let result = [];
        let checkapplykeys: string[] = [];
        for (let applyrule of apply) {
            let applyKey1 = Object.keys(applyrule)[0];
            if (checkapplykeys.includes(applyKey1)) {
                throw new InsightError("applyKey should be unique");
            } else {
            ...
            for (let elements of res) {
                ...
                for (let applyRule of apply) {
                    ...
                    if (applyKey.includes("_")) {
                        throw new InsightError("applyKey shouldn't have _");
                    }
    ...
    private parsewhere(where: any, id: string): any[] {
        /*
        if (Object.keys(where).length === 0) {
            for (let dataset of this.datasets) {
                if (dataset.iid === id) {
                    return this.datasetTonumber(dataset);
                }
            }
        }
        */
        let kind: InsightDatasetKind = InsightDatasetKind.Courses;
        let findDataset: boolean = false;
        for (let dataset of this.datasets) {
            if (dataset.iid === id) {
                kind = dataset.ikind; // get the kind of the current dataset
                findDataset = true;
        ...
        if (where.hasOwnProperty("AND")) {
           // global.console.log("AND");
           // global.console.log(where.AND);
            let parsingData: any[][] = [];
            ...
            let logic: LogicComparison;
            logic = new LogicComparison(Logic.And, parsingData);
            return logic.parseLogic();
        }
        if (where.hasOwnProperty("OR")) {
            global.console.log("OR");
           // global.console.log(where.OR);
            let parsingData: any[][] = [];
            ...
            let logic: LogicComparison;
            logic = new LogicComparison(Logic.Or, parsingData);
            return logic.parseLogic();
        }
        if (where.hasOwnProperty("LT")) {
            global.console.log("LT");
            let dataset = new IDataSet();
            /*
            for (let obj of this.datasets) {
                if (id === obj.iid) {
                    dataset.setkind = obj.ikind;
                    break;
                }
            }*/
            let ikey: key = null;
            ...
            if (Object.keys(where.LT).length !== 1) {
                throw new InsightError("invalid LT");
            }
            ...
            let mc = new MComparison(MComparator.Lt, ikey, value);
        ...
        if (where.hasOwnProperty("GT")) {
            ...
            if (Object.keys(where.GT).length !== 1) {
                throw new InsightError("invalid GT");
            }
            for (let datas of this.datasets) {
                if (datas.iid === id) {
                    for (let sec of datas.ielements) {
            ...
            let mc = new MComparison(MComparator.Gt, ikey, value);
            let parsedata = mc.parseM(dataset);
            if (kind === InsightDatasetKind.Courses) {
                result = InsightFacade.datasetTonumber(parsedata);
            } else {
        ...
        if (where.hasOwnProperty("EQ")) {
            ...
            if (Object.keys(where.EQ).length !== 1) {
                throw new InsightError("invalid EQ");
            }
            for (let datas of this.datasets) {
                if (datas.iid === id) {
                    for (let sec of datas.ielements) {
            ...
            try {
                let mc = new MComparison(MComparator.Eq, ikey, value);
                let parsedata = mc.parseM(dataset);
                if (kind === InsightDatasetKind.Courses) {
                    result = InsightFacade.datasetTonumber(parsedata);
                } else {
        ...
        if (where.hasOwnProperty("IS")) {
            ...
            try {
                let mc = new SComparison(ikey, value);
                ...
                // let parseddata = mc.parseS(dataset);
                // global.console.log(parseddata);
                // result = InsightFacade.datasetTonumber(parseddata);
            } catch (e) {
                throw new InsightError(e);
            }
            return result;
        ...
        if (where.hasOwnProperty("NOT")) {
            ...
            let result: any[];
            /*for (let str of where.NOT) {
                parsingData.concat(this.parsewhere(str));
            }*/
            ...
            try {
                ...
                // result = Negation.parseN(parsingData, datasetNumber2);
                result = Negation.parseN(this.parsewhere(where.NOT, id), allUUID);
            } catch (e) {
                throw new InsightError(e);
            }
            return result;
        }
        throw new InsightError("Empty field");
    }
    public static datasetTonumber(dataset: IDataSet): number[] {
        let result: number[] = [];
    ...
    public static datasetTostring(dataset: IDataSet): string[] {
        let result: string[] = [];
    ...
    public numberTodataset(values: number[], id: string): IDataSet {
        let result: IDataSet = new IDataSet();
        let map: Map<number, ISection> = new Map();
        for (let dataset of this.datasets) {
            if (dataset.iid === id) {
                for (let section of dataset.ielements) {
    ...
    public stringTodataset(values: string[], id: string): IDataSet {
        let result: IDataSet = new IDataSet();
        let map: Map<string, IRoom> = new Map();
        for (let dataset of this.datasets) {
            if (dataset.iid === id) {
                for (let room of dataset.ielements) {
    ...
    /*public gorupby(result: any[], keyset: key[]): Map<string, object[]> {
        let terms: string[] = []; // store the term of each object
        let myMap: Map<string, object[]> = new Map<string, object[]>();
        for (let i in result) {
            let term: string = ""; // term is used as the key to Map
            for (let j in keyset) {
               let val = Object.getOwnPropertyDescriptor(result[i], keyset[j].toString()).value;
               if (typeof val === "number") {
                   val = val.toString();
               }
               term.concat(val); // now each object has its term, eg. cpsc310
            }
            if (terms.length === 0) { // in first term(first object)
                let arr1: object[] = [];
                myMap.set(term, arr1); //
                terms.push(term); // push first term
            } else {
                for (let k in terms) { // k tracks previous terms in terms array
                    if (term === terms[k]) {
                        myMap.get(terms[k]).push(result[i]);
                    } else {
                        let arr: object[] = [];
                        arr.push(result[i]);
                        myMap.set(term, arr);
    ...
    public ielementsToColumns(str: any, ielements: any[], order: any, id: string): any[] {
        let result: IObj[] = [];
        for (let ielement of ielements) {
            let obj: IObj = {};
            if (ielement instanceof ISection) {
                for (let key1 of str) {
            ...
            } else if (ielement instanceof IRoom) {
                for (let key1 of str) {
                    switch (key1.toString()) {
                        case id + "_fullname": {
                ...
                for (let key1 of str) {
                    if (!ielement.hasOwnProperty(key1)) {
                        throw new InsightError("elements in columns not in group or apply");
                    }
